...
struct node_t * insert_node(struct node_t *root, 
                            struct node_t *node);
...
struct node_t * make_new_node(struct node_t *left, 
                                struct node_t *right);
...
void make_code_table(struct node_t *root, 
                    code_table_t *table);

// возвращает код символа по дереву
unsigned char get_char_code(unsigned char symbol, 
                            struct node_t *root);

void generate_codes(struct node_t *node, 
                    code_table_t *table, 
